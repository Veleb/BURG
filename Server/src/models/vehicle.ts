...
const VehicleSchema = new Schema<VehicleInterface>(
  {
    ...
    details: {
      ...
      tires: { type: Number, default: 4, min: 2 },
...
VehicleSchema.pre("save", async function (next) {
  const vehicle = this;
  if (
    !vehicle.isModified("details.name") &&
    !vehicle.isModified("details.model")
  )
    return next();

  const baseSlug = slugify(
    `${vehicle.details.name}-${vehicle.details.model}-${vehicle.details.year}`,
    {
      lower: true,
      strict: true,
    }
